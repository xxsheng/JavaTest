(function() {
	...
	var tipController = function($scope, $state, $ionicModal, $location, $window, $ionicSideMenuDelegate, postDataService){
		$scope.getSysNoticeList = function(){
			postDataService.getPostData({}, '/SysNoticeList').then(function(data) {
	...
	var lotteryTrendController = function($scope, $timeout, $state, postDataService) {
		...
		$scope.lotteryName = $state.params.name;
		$scope.displayLost = true;
		$scope.displayLostLine = true;
		$scope.displayChart = true;
		$scope.searchObj = {lotteryId: $scope.lotteryId, command: 'latest-30-desc'};

		$scope.search = function(command) {
			if(command)
				$scope.searchObj.command = command;
			
			postDataService.getPostData($scope.searchObj, '/LotteryCodeTrend').then(function(data) {
				switch (data.data.lottery.type) {
					case 1:
					case 7:
						$scope.layout = {
				...
				$.each(data.data.list, function(itemIndex, item){
					var codes = item.code.split(',');
					codeSet.push(codes);
				});
				$scope.codeLost = calCodeLost($scope.layout, codeSet);
				$scope.codeLostLine = calCodeLostLine($scope.layout, $scope.codeLost);
		...
		$scope.initTable = function(){
			$scope.table = $('.trendTable').DataTable({
				searching:false,
				info:false,
				ordering:false,
		        scrollY:(document.documentElement.clientHeight-193)+"px",
		        scrollX:true,
		        scrollCollapse:true,
		        paging:false,
		        fixedColumns:{
		...
		$scope.togglerLost = function(){
			if($scope.displayLost){
				$scope.displayLost = false;
				$('.trendTable tbody td.content i.lost').css("visibility", "hidden");
			}else{
				$scope.displayLost = true;
				$('.trendTable tbody td.content i.lost').css("visibility", "visible");
			}
		};
		$scope.togglerLostLine = function(){
			if($scope.displayLostLine){
				$scope.displayLostLine = false;
				$('.trendTable tbody td.content span.line').css("background-color","transparent");
			}else{
				$scope.displayLostLine = true;
				$('.trendTable tbody td.content span.line').css("background-color","#e1f9f7");
			}
		};
		$scope.togglerChart = function(){
			if($scope.displayChart){
				$(".dataTables_scrollBody canvas").hide();
				$scope.displayChart = false;
			}else{
				$scope.displayChart = true;
				$(".dataTables_scrollBody canvas").show();
			}
		};
		...
		function renderLines() {
			$(".dataTables_scrollBody canvas").remove();
			var canvas = $("<canvas style='position: absolute;z-index:1'/>")[0];
			$(".dataTables_scrollBody").prepend(canvas);
			canvas.width = $('.dataTables_scrollBody table').width()
			canvas.height = $('.dataTables_scrollBody table').height()
			
			var diffOffset = $(".dataTables_scroll .dataTables_scrollBody .dataTable").offset();
			var context = canvas.getContext("2d")
			$.each($scope.layout.title, function(i) {
				context.lineWidth = 1.5
				context.strokeStyle = '#f53f00'
				$('.trendTable tbody td.content.col'+i).each(function(n) {
					var $dom = $('i.ball', this)
					var offset = $dom.offset()
					var x = offset.left + $dom.width() / 2 - diffOffset.left;
					var y = offset.top + $dom.height() / 2 - diffOffset.top;
	...
	var chargeController = function($scope, shareDataService, postDataService, $state, $ionicPopup) {
		...
		vm.changeTabs = function(type) {
			...
			if(vm.showChannelList.length > 0) {
				...
				if(vm.selectedChannel.type==1 &&vm.selectedChannel.subType==2){
	...
	var agentController = function($scope, $q, $filter, $ionicModal, $timeout, shareDataService, postDataService, lotteryUtilsService, $state, $ionicPopup, passwordService) {
		...
		vm.recordLoadmore = function(page) {
			...
			vm.recordObj = {
				...
				start: (vm.page - 1) * 10,
				sTime: $filter('date')(vm.searchObj.sTime, 'yyyy-MM-dd'),
				eTime: $filter('date')(getNetTime(vm.searchObj.eTime), 'yyyy-MM-dd'),
				lotteryId: vm.searchObj.lotteryId,
				status: vm.searchObj.status,
				account: vm.searchObj.account,
				expect: vm.searchObj.expect,
