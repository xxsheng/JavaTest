...
public class HighOpenTimeUtil implements OpenTimeUtil {
		...
		if(lottery != null) {
			...
			for (int i = 0, j = list.size(); i < j; i++) {
				...
				if(currTime.compareTo(startTime) >= 0 && currTime.compareTo(stopTime) < 0) {
					...
					if("mlaft".equals(lottery.getShortName())){
						return getMlaftOpenTime(bean);
					}
					return bean;
				}
			}
	...
	public List<OpenTime> getOpenTimeList(int lotteryId, int count) {
		...
		if(lottery != null) {
			...
			for (int i = 0; i < count; i++) {
				...
				if(tmpBean != null) {
					if("mlaft".equals(lottery.getShortName())){
						tmpBean = getMlaftOpenTime(tmpBean);
					}
					list.add(tmpBean);
				}
				// 计算下一期
	...
	/**
	 * 重置幸运飞艇期号，处理跨天问题
	 * mlaft	131	23:54:00	23:59:00	23:59:30
	 *	mlaft	132	23:59:00	00:04:00	00:04:30
	 * @param lotteryId
	 * @param currTime
	 * @return
	 */
	private OpenTime getMlaftOpenTime(OpenTime mlaft) {
		String expect = mlaft.getExpect();
		int expectNo = Integer.valueOf(expect.substring(9));
		if(expectNo > 131){
			String yday = DateUtil.getYesterday();
			expect = yday.replaceAll("-", "") + expect.substring(8);
			mlaft.setExpect(expect);
		}
		return mlaft;
	}
}