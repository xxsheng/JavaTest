...
public class LotteryOpenUtil {
	...
	public OpenTime substractOneExpect(int lotteryId, String expect) {
		...
		if(lottery != null) {
			String subExpect;
			if (expect.indexOf("-") <= -1) {
				Integer integer = Integer.valueOf(expect);
				integer -= 1;
				subExpect = integer.toString();
			}
			else {
				int formatCount = 3;
				if (lotteryId == 118 || lotteryId == 122|| lotteryId == 123) {
					formatCount = 4;
				}
				...
				if (currExpect.equals("001") || currExpect.equals("0001")) {
					...
					subExpect = String.format("%0"+formatCount+"d", 1);
				}
				else {
				...
				subExpect = date.replaceAll("-", "") + "-" + subExpect;
			}

			OpenTime tmpBean = getOpentime(lotteryId, subExpect);
			return tmpBean;
		}
		return null;
	}
