...
public class UserWithdrawLogController extends AbstractActionController {
	...
	@RequestMapping(value = WUC.LOTTERY_USER_WITHDRAW_LOG, method = { RequestMethod.POST })
	@ResponseBody
	public void LOTTERY_USER_UPDATE_LOG(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
		String actionKey = WUC.LOTTERY_USER_WITHDRAW_LOG;
		...
		if (uEntity != null) {
			if (super.hasAccess(uEntity, actionKey)) {
				...
				PageList pList = userWithdrawLogService.search(billno,username,start, limit);
				if(pList != null) {
