...
public class AdminUserActionLogController extends AbstractActionController {
	...
	@RequestMapping(value = WUC.ADMIN_USER_ACTION_LOG_LIST, method = { RequestMethod.POST })
	@ResponseBody
	public void ADMIN_USER_ACTION_LOG_LIST(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
		String actionKey = WUC.ADMIN_USER_ACTION_LOG_LIST;
		...
		if (uEntity != null) {
			if (super.hasAccess(uEntity, actionKey)) {
				...
				PageList pList = adUserActionLogService.search(username, actionId, error, sTime, eTime, start, limit);
				if(pList != null) {
